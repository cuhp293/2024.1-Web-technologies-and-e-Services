...
function loadAdminMenuTop() {
  let newMenu = menu.filter(
    (item) => item.id !== "menu1" && item.id !== "menu5"
  );
  ...
  document.getElementById(
    "admin-page-title"
  ).innerHTML = `Admin menu top: "Chỉnh sửa menu top"`;
  ...
  let newStudentInfo = [
    {
      ...
      details: [
        ...
        },
        {
          ...
          html: `
                <div class="details">
                  <span>Họ và tên:</span> Đinh Thị Hồng Phúc
                </div>
                ...
                <div class="details"><span>MSSV:</span> 20215118</div>
                ...
                <div class="details">
                  <span>Email:</span> phuc.dth215118@sis.hust.edu.vn
                </div>
                `,
        },
      ],
          ...
          html: `
                <table class="table-info">
                  <tr>
                    <th>KPInsight</th>
                    <td>
                      Mô tả: Hệ thống cung cấp các giải pháp cho phép người dùng
                      theo dõi và quản lý KPI hiệu quả hơn bằng cách tính toán và
                      phân tích kết quả công việc. Nhờ đó, người dùng có thể điều
                      chỉnh công việc của mình chính xác hơn và đạt được mục tiêu
                      một cách hiệu quả nhất.
                      <br />Công nghệ sử dụng: HTML、CSS、git、Jira、ReactJS
                      <br />Vai trò: Thiết kế giao diện người dùng và phát triển
                  ...
                  <tr>
                    <th>TravelAI</th>
                    <td>
                      Mô tả: Dự án này sử dụng thuật toán A* để tìm ra các tuyến đường ngắn nhất giữa các địa điểm du lịch.
                      Thuật toán A* là một trong những thuật toán tìm kiếm thông minh nhất và sử dụng hàm đánh giá để xác định đường đi tối ưu từ điểm bắt đầu đến điểm kết thúc.
                      <br />Công nghệ sử dụng: Python、Django、PostgreSQL、git <br />Vai
                      trò: Thiết kế giao diện người dùng và phát triển
                      một số tính năng của hệ thống.
                    </td>
                  </tr>
                  <tr>
                    <th>Photo Filter</th>
                    <td>
                      Mô tả: "Photo Filter" là một ứng dụng Android cho phép người
                      dùng tải lên và chỉnh sửa ảnh trực tiếp trên thiết bị di
                      động của mình. Người dùng có thể áp dụng các bộ lọc khác
                      nhau, điều chỉnh màu sắc và thêm hiệu ứng đặc biệt để tạo ra
                      những bức ảnh đẹp và độc đáo. <br />Công nghệ sử dụng: Java
                      (Phát triển ứng dụng), Android SDK (Sử dụng các thành phần
                      giao diện của Android để xây dựng UI và UX), thư viện xử lý
                      ảnh (Sử dụng các thư viện xử lý ảnh như Picasso, PhotoEditor
                      để áp dụng các hiệu ứng và bộ lọc). <br />Vai trò: Thiết kế
                      giao diện người dùng và phát triển một số tính năng của hệ
                      thống.
                    </td>
                  </tr>
                </table>     
...
function loadAdminMenuLeft(sectionId) {
  ...
  document.getElementById("admin-menu").innerHTML = loadMenuTable(
    arr,
    "loadAdminContentsLayout",
    sectionId
  );
}
...
function loadAdminContentsLayout(id) {
  ...
  document.getElementById(
    "admin-page-title"
  ).innerHTML = `Admin contents layout: "${title1.title}/${title2.title}" <a target="_blank" href="html/layout-help.html"><i class="fa-solid fa-circle-info"></i></a>`;
  document.getElementById("admin-menu").innerHTML = loadMenuTable(
    details,
    "loadAdminContents",
    id
  ...
  document.getElementById("preview-layout").innerHTML =
    html + loadGridContents(8, 15);
}
...
function loadAdminContents(id) {
  ...
  document.getElementById(
    "admin-page-title"
  ).innerHTML = `Admin contents: "${title1.title}/${title2.title}/${title3.title}"`;
...
function loadSidebar(sectionId) {
  ...
  sidebar.innerHTML =
    `<h4 class="w3-bar-item"><b>${item.title}</b></h4>` + sidebarHTML(arr);
}
...
function saveEditName(id) {
  ...
  if (id.includes("-content")) {
    parts = id.split("-");
    let arr = getArr(id);
    let details = arr.find(
      (item) => item.id === parts[0] + "-" + parts[1]
    ).details;
...
function saveEditMedia(id) {
  ...
  mediaQueries.forEach((mediaQuery) => {
    let mediaCondition = mediaQuery.querySelector(
      ".media-condition-input"
    ).value;
...
function deleteMenu(id) {
  ...
  if (id.includes("-content")) {
    ...
    if (arr === menu) {
      details = arr.find(
        (item) => item.id === convertSectionId(sectionId)
      ).details;
      ...
      <th class="width-80">
        <div class="name-input-div">
          <div class="flex space-between">
            Tên: 
            <div class="flex">
            <i class="fa-xl fa-solid fa-xmark" onclick="cancelAdd('${
              rowIndex + 1
            }')"></i>
            <i class="fa-xl fa-solid fa-check" onclick="saveAdd('${
              rowIndex + 1
            }', '${id}')"></i>
            </div>
          </div>
          <textarea rows="2" class="name-input full-width" id="input-${
            rowIndex + 1
          }"> </textarea>
        </div>
      </th>
...
function saveAdd(index, id) {
  ...
  if (id.includes("-content")) {
    ...
    document.getElementById("preview-layout").innerHTML =
      html + layout.outerHTML + loadGridContents(8, 15);
  } else {
    let arr = [];
    let key = "";
    if (!id.includes("-detail")) {
  ...
  for ([conditionText, cssText] of mediaQueriesContainingId) {
    ...
    <tr>
      <td class="media-query">
        ...
        <textarea rows="4" class="media-css-input full-width">${extractCssContent(
          cssText
        )}</textarea>
        }
      </td>
